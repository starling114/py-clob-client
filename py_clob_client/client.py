...
class ClobClient:
    ...
    def get_ok(self):
        ...
        """
        return get("{}/".format(self.host), headers=self._merge_headers(), proxy=self.proxy)
    ...
    def get_server_time(self):
        ...
        Does not need authentication
        """
        return get("{}{}".format(self.host, TIME), headers=self._merge_headers(), proxy=self.proxy)
    ...
    def create_api_key(self, nonce: int = None) -> ApiCreds:
        ...
        creds_raw = post(endpoint, headers=self._merge_headers(headers), proxy=self.proxy)
        try:
            creds = ApiCreds(
    ...
    def derive_api_key(self, nonce: int = None) -> ApiCreds:
        ...
        creds_raw = get(endpoint, headers=self._merge_headers(headers), proxy=self.proxy)
        try:
            creds = ApiCreds(
                api_key=creds_raw["apiKey"],
                api_secret=creds_raw["secret"],
    ...
    def get_api_keys(self):
        ...
        return get("{}{}".format(self.host, GET_API_KEYS), headers=self._merge_headers(headers), proxy=self.proxy)
    ...
    def get_closed_only_mode(self):
        ...
        headers = create_level_2_headers(self.signer, self.creds, request_args)
        return get("{}{}".format(self.host, CLOSED_ONLY), headers=self._merge_headers(headers), proxy=self.proxy)
    ...
    def delete_api_key(self):
        ...
        headers = create_level_2_headers(self.signer, self.creds, request_args)
        return delete("{}{}".format(self.host, DELETE_API_KEY), headers=self._merge_headers(headers), proxy=self.proxy)

    def get_midpoint(self, token_id):
        """
        Get the mid market price for the given market
        """
        return get("{}{}?token_id={}".format(self.host, MID_POINT, token_id), headers=self._merge_headers(), proxy=self.proxy)
    ...
    def get_midpoints(self, params: list[BookParams]):
        ...
        return post("{}{}".format(self.host, MID_POINTS), data=body, headers=self._merge_headers(), proxy=self.proxy)

    def get_price(self, token_id, side):
        """
        Get the market price for the given market
        """
        return get("{}{}?token_id={}&side={}".format(self.host, PRICE, token_id, side), headers=self._merge_headers(), proxy=self.proxy)
    ...
    def get_prices(self, params: list[BookParams]):
        ...
        return post("{}{}".format(self.host, GET_PRICES), data=body, headers=self._merge_headers(), proxy=self.proxy)

    def get_spread(self, token_id):
        """
        Get the spread for the given market
        """
        return get("{}{}?token_id={}".format(self.host, GET_SPREAD, token_id), headers=self._merge_headers(), proxy=self.proxy)
    ...
    def get_spreads(self, params: list[BookParams]):
        ...
        """
        body = [{"token_id": param.token_id} for param in params]
        return post("{}{}".format(self.host, GET_SPREADS), data=body, headers=self._merge_headers(), proxy=self.proxy)

    def get_tick_size(self, token_id: str) -> TickSize:
        if token_id in self.__tick_sizes:
            return self.__tick_sizes[token_id]

        result = get("{}{}?token_id={}".format(self.host, GET_TICK_SIZE, token_id), headers=self._merge_headers(), proxy=self.proxy)
    ...
    def get_neg_risk(self, token_id: str) -> bool:
        ...
        result = get("{}{}?token_id={}".format(self.host, GET_NEG_RISK, token_id), headers=self._merge_headers(), proxy=self.proxy)
    ...
    def get_fee_rate_bps(self, token_id: str) -> int:
        ...
        result = get("{}{}?token_id={}".format(self.host, GET_FEE_RATE, token_id), headers=self._merge_headers(), proxy=self.proxy)
        ...
        if self.can_builder_auth():
            ...
            if builder_headers is not None:
                return post(
                    ...
                    headers=self._merge_headers(builder_headers),
                    data=body,
                    proxy=self.proxy,
                )
        return post("{}{}".format(self.host, POST_ORDERS), headers=self._merge_headers(headers), data=body, proxy=self.proxy)
    ...
    def post_order(self, order, orderType: OrderType = OrderType.GTC):
        ...
        return post("{}{}".format(self.host, POST_ORDER), headers=self._merge_headers(headers), data=body, proxy=self.proxy)
    ...
    def cancel(self, order_id):
        ...
        headers = create_level_2_headers(self.signer, self.creds, request_args)
        return delete("{}{}".format(self.host, CANCEL), headers=self._merge_headers(headers), data=body, proxy=self.proxy)
    ...
    def cancel_orders(self, order_ids):
        ...
        headers = create_level_2_headers(self.signer, self.creds, request_args)
        return delete(
            "{}{}".format(self.host, CANCEL_ORDERS), headers=self._merge_headers(headers), data=body, proxy=self.proxy
        )
    ...
    def cancel_all(self):
        ...
        headers = create_level_2_headers(self.signer, self.creds, request_args)
        return delete("{}{}".format(self.host, CANCEL_ALL), headers=self._merge_headers(headers), proxy=self.proxy)
    ...
    def cancel_market_orders(self, market: str = "", asset_id: str = ""):
        ...
        headers = create_level_2_headers(self.signer, self.creds, request_args)
        return delete(
            "{}{}".format(self.host, CANCEL_MARKET_ORDERS), headers=self._merge_headers(headers), data=body, proxy=self.proxy
        )
    ...
    def get_orders(self, params: OpenOrderParams = None, next_cursor="MA=="):
        ...
        while next_cursor != END_CURSOR:
            ...
            response = get(url, headers=self._merge_headers(headers), proxy=self.proxy)
    ...
    def get_order_book(self, token_id) -> OrderBookSummary:
        ...
        """
        raw_obs = get("{}{}?token_id={}".format(self.host, GET_ORDER_BOOK, token_id), headers=self._merge_headers(), proxy=self.proxy)
    ...
    def get_order_books(self, params: list[BookParams]) -> list[OrderBookSummary]:
        ...
        """
        body = [{"token_id": param.token_id} for param in params]
        raw_obs = post("{}{}".format(self.host, GET_ORDER_BOOKS), data=body, headers=self._merge_headers(), proxy=self.proxy)
    ...
    def get_order(self, order_id):
        ...
        headers = create_level_2_headers(self.signer, self.creds, request_args)
        return get("{}{}".format(self.host, endpoint), headers=self._merge_headers(headers), proxy=self.proxy)
    ...
    def get_last_trade_price(self, token_id):
        ...
        """
        return get("{}{}?token_id={}".format(self.host, GET_LAST_TRADE_PRICE, token_id), headers=self._merge_headers(), proxy=self.proxy)
    ...
    def get_last_trades_prices(self, params: list[BookParams]):
        ...
        """
        body = [{"token_id": param.token_id} for param in params]
        return post("{}{}".format(self.host, GET_LAST_TRADES_PRICES), data=body, headers=self._merge_headers(), proxy=self.proxy)
    ...
    def are_orders_scoring(self, params: OrdersScoringParams):
        ...
        headers = create_level_2_headers(self.signer, self.creds, request_args)
        return post(
            "{}{}".format(self.host, ARE_ORDERS_SCORING), headers=self._merge_headers(headers), data=body, proxy=self.proxy
        )
    ...
    def get_sampling_markets(self, next_cursor="MA=="):
        ...
        """
        return get(
            "{}{}?next_cursor={}".format(self.host, GET_SAMPLING_MARKETS, next_cursor), headers=self._merge_headers(), proxy=self.proxy
        )
    ...
    def get_sampling_simplified_markets(self, next_cursor="MA=="):
        ...
        return get(
            ...
            ), headers=self._merge_headers(), proxy=self.proxy
        )
    ...
    def get_markets(self, next_cursor="MA=="):
        ...
        """
        return get("{}{}?next_cursor={}".format(self.host, GET_MARKETS, next_cursor), headers=self._merge_headers(), proxy=self.proxy)
    ...
    def get_simplified_markets(self, next_cursor="MA=="):
        ...
        """
        return get(
            "{}{}?next_cursor={}".format(self.host, GET_SIMPLIFIED_MARKETS, next_cursor), headers=self._merge_headers(), proxy=self.proxy
        )
    ...
    def get_market(self, condition_id):
        ...
        """
        return get("{}{}{}".format(self.host, GET_MARKET, condition_id), headers=self._merge_headers(), proxy=self.proxy)

    def get_market_trades_events(self, condition_id):
        """
        Get the market's trades events by condition id
        """
        return get("{}{}{}".format(self.host, GET_MARKET_TRADES_EVENTS, condition_id), headers=self._merge_headers(), proxy=self.proxy)
